...
01、新建：如果当前有内容，且有更改，则询问是否保存，然后在新建。

02、打开：如果当前有内容，且有更改，则询问是否保存，然后在打开。

03、保存：如果有文件名，则直接保存，如果没有，则打开保存文件对话框用户选择保存。

04、另存：打开保存对话框，用户自定义，另存一份文件。

05、打印：调用打印驱动，然后打印内容。

06、撤销：撤销上次操作。

07、剪切：这可以直接和 `QTextEdit` 的相绑定
...
14、放大：如果选中图片，则放大图片，如果选中文字，则放大文字，如果都选中，两者都放大。
...
19、字体：设置选中及新增字为当前选中字体

20、字号：设置选中及新增字为当前选中字号

21、粗体：设置选中及新增字为加粗，或者取消加粗。如果当前为粗体，则取消加粗，显示未选择状态；如果为正常体，则加粗，**并显示为选中状态。**

22、斜体：设置选中及新增字为斜体，或者取消斜体。如果当前为斜体，则取消斜体，显示未选择状态；如果为正常体，则斜体，**并显示为选中状态。**

23、下划线：设置选中及新增字增加下划线。如果当前有下划线，则取消，显示未选择状态；如果为正常体，则增加下划线，**并显示为选中状态。**

24、颜色：为选中部分和后续设置当前颜色，最好控件能改变颜色，这样一下就能看出来当前为什么颜色。
...
设置字体大小，这里还需要看一下这个`QTextEdit` 的结构
